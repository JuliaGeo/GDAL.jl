...
tinydata = UInt8[128 126 161 185 193
                 171 164 175 206 205
                 127 148 177 205 209
                 93 114 164 172 181
                 83 101 140 128 122]
...
ds_warped = GDAL.gdalwarp("data/utmsmall.mem",
...
ds_dempr = GDAL.gdaldemprocessing("data/utmtiny-hillshade.asc",
                                  ds_tiny,
                                  "hillshade",
                                  C_NULL,
                                  options,
...
ds_nearblack = GDAL.gdalnearblack("data/utmtiny-nearblack.tif",
                                  Ptr{GDAL.GDALDatasetH}(C_NULL),
                                  ds_tiny,
                                  options,
                                  C_NULL)
...
# GDALGrid
options = GDAL.gdalgridoptionsnew(["-of", "MEM", "-outsize", "3", "10", "-txe", "100", "100.3", "-tye", "0", "0.1"],
                                  C_NULL)
...
ds_rasterize = GDAL.gdalrasterize("data/point-rasterize.mem",
                                  Ptr{GDAL.GDALDatasetH}(C_NULL),
                                  ds_point,
                                  options,
                                  C_NULL)
...
@test occursin("<SourceFilename relativeToVRT=\"1\">utmtiny.tif</SourceFilename>",
...
ds_csv = GDAL.gdalvectortranslate("data/point.csv",
                                  Ptr{GDAL.GDALDatasetH}(C_NULL),
                                  1,
                                  [ds_point],
                                  options,
