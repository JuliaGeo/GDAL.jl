...
function brief_description(node::EzXML.Node)
    # brief description, always 1 present, sometimes only whitespace
    return strip(nodecontent(findfirst("briefdescription", node)))
end
...
function text(node::EzXML.Node, el::AbstractString)
    s = findfirst(el, node)
    return s === nothing ? "" : strip(nodecontent(s))
end
...
function findnode(name::String, doc::EzXML.Document)
    ...
    nodes = findall("$memberdef[translate(name, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz')='$name' and $nofriend]",
                    doc)
...
"Parse a julia file that is not a single expression by wrapping it in a quote"
function parsefile(path)
    return Meta.parse(join(["quote", read(path, String), "end"], ";"))
end

"Loop over expressions to add docstrings to a script"
function add_doc(lib_path)
    code = parsefile(lib_path)
    code = MacroTools.prewalk(rmlines, code)
    body = code.args[1].args

    rewritten = []
    for ex in body
        if Meta.isexpr(ex, :macrocall) && ex.args[1] == GlobalRef(Core, Symbol("@doc"))
            # keep docstrings created by Clang.jl's `extract_c_comment_style = "doxygen"`
            push!(rewritten, addquotes(ex.args[3]), ex.args[4])
        else
            # otherwise build docstrings from the Doxygen XML
            name = cname(ex)
    ...
    open(lib_path, "w") do io
        println(io,
                "# This file is automatically generated by gen/generator.jl,",
